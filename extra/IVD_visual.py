...
def compute_adjacent_vertebra_pairings(mesh_dir, compute_suffix, n_sample=30000, n_pairs=500,
                                       max_dist=8.0, seed=42, outlier_k=2.5):

    np.random.seed(seed)

    # load and sort meshes for computation
    files = [f for f in os.listdir(mesh_dir) if f.endswith(compute_suffix) and re.search(r"L\d+", f)]
...
def load_visualization_meshes(mesh_dir):
    """Load meshes that end in _L1, _L2, etc for visualization"""
    files = [f for f in os.listdir(mesh_dir) if re.search(r"L\d+\.vtk$", f)]
    meshes = {}
    
    for f in files:
        level = parse_level(f)
        meshes[level] = load_mesh(os.path.join(mesh_dir, f))
...
def visualize_pairings(mesh_i, mesh_j, pairs_i, pairs_j, subsample=200):
    ...
    plotter = pv.Plotter()
    plotter.add_mesh(mesh_i, color="lightgray", opacity=0.6)
    plotter.add_mesh(mesh_j, color="lightblue", opacity=0.6)
...
if __name__ == "__main__":
    ...
    mesh_dir = "/Users/elise/elisedonszelmann-lund/Masters_Utils/Pig_Data/pig2/Registration/CT_segmentations/cropped/sofa3"
    
    # Compute pairings using these meshes
    # compute_suffix = "_upper.vtk"
    compute_suffix = '_body.vtk'
    
    # Compute pairings - unpack the tuple
    pairings, _ = compute_adjacent_vertebra_pairings(
        mesh_dir,
        compute_suffix,
        n_sample=30000,
        n_pairs=500,
    ...
    # Load visualization meshes (_L1, _L2, etc.)
    viz_meshes = load_visualization_meshes(mesh_dir)

    # Visualize using the different meshes
