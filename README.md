...
A GPU-accelerated, intensity-based groupwise registration framework for aligning preoperative CT to intraoperative ultrasound (iUS) in spinal surgery. Achieves clinically relevant accuracy (0.99 ± 70 mm mean TRE) with clinical speed (~10s) while maintaining robustness under large initial misalignment and missing ultrasound data.
